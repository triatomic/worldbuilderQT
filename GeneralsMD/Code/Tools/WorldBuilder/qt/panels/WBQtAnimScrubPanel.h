...
//
// The slider steps over the frames of the LONGEST animation on the selection, so a step always
// lands on a real frame. What crosses the facade is still a normalized 0..1 position: one object's
// modules run animations of different lengths, so a single frame number would mean a different
// point in each. All calls go through the WBQtAnimScrub_* facade.
...
private:
	...
	// The slider counts frames; the facade takes a 0..1 position. These convert between the two,
	// and are the only places that know the slider's range is the frame count.
	int lastFrame() const;
	double frameToFraction(int frame) const;
	int fractionToFrame(double fraction) const;
